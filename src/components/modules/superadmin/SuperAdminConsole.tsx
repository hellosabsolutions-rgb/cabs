...
export const SuperAdminConsole: React.FC = () => {
  ...
  const openBizDetail = async (id: string) => {
    setSelectedBizId(id);
    const detail = await superAdminApi.getBusinessDetail(id);
    if (detail) {
  ...
  const handleToggleBizStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      // Refresh audit logs
      const updatedAudit = await superAdminApi.getAuditLogs();
  ...
  const handleRetryPayment = async (biz: string) => {
    try {
      await superAdminApi.retryPayment(biz);
      ...
      <div className="sa-app">
        ...
        <div className="sa-main">
          ...
          <div className="sa-content">
              ...
              <div className="sa-view active">
                <div className="sa-filters-row">
                  ...
                  <button
                    ...
                    onClick={() => alert('New Business Onboarding Triggered')}
                  >
            ...
            {/* ============ 3. BUSINESS DETAIL (360° DRILLDOWN) ============ */}
              ...
              <div className="sa-view active">
                <div className="sa-settings-grid">
                  <div className="sa-settings-nav">
                    {['general', 'notifications', 'security', 'billing', 'team'].map(s => (
                      <div
                        key={s}
                        className={`sa-s-item ${activeSetTab === s ? 'active' : ''}`}
                        onClick={() => setActiveSetTab(s)}
                      >
                        {s === 'general' ? 'General' : s === 'notifications' ? 'Notifications' : s === 'security' ? 'Security' : s === 'billing' ? 'Billing Info' : 'Team Access'}
                      </div>
                    ))}
                  </div>
