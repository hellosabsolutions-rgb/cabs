                ...
                <tbody>
                  ...
                  {vehicleTrips.map(t => (
                    <tr key={t.id} style={{ borderBottom: '1px solid var(--border, rgba(255,255,255,0.05))' }}>
                      ...
                      <td style={{ padding: '10px 12px', fontWeight: 600, color: 'var(--success)' }}>
                        ₹{(t.revenue || t.totalAmount || 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '10px 12px' }}>
                  ...
                  {vehicleDutyLogs.map(d => (
                    <tr key={d.id} style={{ borderBottom: '1px solid var(--border, rgba(255,255,255,0.05))' }}>
                      ...
                      <td style={{ padding: '10px 12px', color: 'var(--text-muted)' }}>
                        {d.journeyFrom && d.journeyTo ? `${d.journeyFrom} → ${d.journeyTo}` : d.dutyType || 'Official Duty'}
                      ...
                      <td style={{ padding: '10px 12px', fontWeight: 600, color: 'var(--text)' }}>
                        {d.tollParkingAmount ? `Toll: ₹${d.tollParkingAmount}` : 'Contracted'}
                      </td>
                      <td style={{ padding: '10px 12px' }}>
                  ...
                  <tbody>
                    {vehicleFuelLogs.map(f => (
                      <tr key={f.id} style={{ borderBottom: '1px solid var(--border, rgba(255,255,255,0.05))' }}>
                        ...
                        <td style={{ padding: '8px 10px', color: 'var(--text)' }}>{f.litres} L</td>
                        <td style={{ padding: '8px 10px', fontWeight: 600, color: '#f87171' }}>₹{f.totalCost?.toLocaleString()}</td>
                        <td style={{ padding: '8px 10px', color: 'var(--text-faint)' }}>{f.odometer ? `${f.odometer} km` : '—'}</td>
                        <td style={{ padding: '8px 10px', color: 'var(--text-muted)' }}>{f.driverName || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {vehicleFastagTolls.map(t => (
                      <tr key={t.id} style={{ borderBottom: '1px solid var(--border, rgba(255,255,255,0.05))' }}>
                        ...
                        <td style={{ padding: '8px 10px', color: 'var(--text-muted)' }}>{t.tollPlaza || 'Toll Plaza'}</td>
